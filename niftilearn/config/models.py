...
class DataConfig(BaseModel):
    ...
    annotation_dir: Path = Field(..., description="Path to annotation directory")
    ...
    test_split: float = Field(0.1, ge=0.0, le=1.0, description="Test split ratio")
    
    # Slice processing parameters
    slice_axis: Literal[0, 1, 2] = Field(2, description="Axis for slice extraction")
    batch_size: int = Field(8, gt=0, description="Batch size for training")
    img_size: List[int] = Field([224, 224], description="Target 2D slice size [H, W]")
    
    # Volume processing parameters
    target_spacing: List[float] = Field(
        [1.0, 1.0, 1.0], 
        description="Target voxel spacing [x, y, z]"
    )
    ...
    use_adaptive_hu_normalization: bool = Field(
        True, 
        description="Use adaptive HU normalization based on percentiles"
    )
    ...
    hu_window_preset: Optional[Literal["soft_tissue", "bone", "lung"]] = Field(
        None, 
        description="Predefined HU window preset"
    )
...
class ComputeConfig(BaseModel):
    ...
    strategy: Literal["ddp", "fsdp", "auto"] = Field(
        "ddp", 
        description="Multi-GPU strategy"
    )
...
class ModelConfig(BaseModel):
    ...
    img_size: List[int] = Field([224, 224], description="Input image size [H, W]")
    ...
    spatial_dims: Literal[2] = Field(
        2, 
        description="Spatial dimensions (fixed to 2D for slice processing)"
    )
...
class TrainingConfig(BaseModel):
    ...
    weight_decay: float = Field(0.01, ge=0.0, description="Weight decay for optimizer")
    ...
    min_delta: float = Field(1e-4, ge=0.0, description="Minimum change for improvement")
